...
def start_game():
    global player_group, obstacle_group, game_active
    game_active = True
    player_group.sprite.reset() # reset player
    obstacle_group.empty() # clear group
    obstacle_group.add(Obstacle(screen_rect.midright)) # add back the asteroid
    bg_music.play(-1) 


class Player(pygame.sprite.Sprite): # Class derived from Sprite for spaceship
    def __init__(self):
        super().__init__()
        self.original_image = pygame.image.load('images/spaceship.png').convert_alpha() # store original image for rotating
        self.image = self.original_image # self.image is required for a pygame Sprite
        self.rect = self.image.get_rect(center = screen_rect.center) # self.rect is required for pygame Sprite
    ...
    def update(self): # update is called when update() is called on the sprite group
        self.rotate()
        self.accelerate()
        self.apply_drag()
        self.move()
    ...
    def reset(self): # reset the ship to initial values
        self.acceleration = 0
        self.velocityX = 0
        self.velocityY = 0
        self.angle = 0
...
class Obstacle(pygame.sprite.Sprite): # class derived from Sprite for Asteroids
...
#-------- Game Active -----------------------
player_group = pygame.sprite.GroupSingle() # create GroupSingle for player
player_group.add(Player()) # add a player object to the player group

obstacle_group = pygame.sprite.Group() # create Group for obstacles
obstacle_group.add(Obstacle((screen_rect.midright))) # add an asteroid to the group
...
while True:
    if game_active:                       
        ...
        player_group.update()  # update player group
        player_group.draw(screen) # draw player group

        obstacle_group.update() # update obstacle group
        obstacle_group.draw(screen) # draw obstacle group
