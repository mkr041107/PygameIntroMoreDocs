...
game_active = False # boolean to determine if the game is running
start_font = pygame.font.Font(None, 64) #font for starting text
start_display = start_font.render("Press Space to begin", True, "white", None) # render text surface
start_display_rect = start_display.get_rect(center = (SCREEN_WIDTH//2, SCREEN_HEIGHT//2)) # create rect for text surface 
                                                                                          # centered at the center of the screen

clock = pygame.time.Clock() 


while True:
    if game_active:                       # if statement to check new game_active bool
    ...
    else: # what happens when the game is not active
        screen.fill((0,0,0))
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
